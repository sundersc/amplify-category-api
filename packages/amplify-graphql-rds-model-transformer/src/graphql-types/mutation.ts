...
import { makeConditionInput, makeModelInputType } from './common';
 ...
 */
export const makeModelConditionInput = (
  ctx: TransformerTransformSchemaStepContextProvider,
  name: string,
  object: ObjectTypeDefinitionNode,
): InputObjectTypeDefinitionNode => {
  const input = makeConditionInput(ctx, name, object);
  return input.serialize();
};

/**
 * makeMutationInputType
 */
export const makeMutationModelInputType = (
  ctx: TransformerTransformSchemaStepContextProvider,
  name: string,
  object: ObjectTypeDefinitionNode,
): InputObjectTypeDefinitionNode => {
  const input = makeModelInputType(ctx, name, object);
  return input.serialize();
};
