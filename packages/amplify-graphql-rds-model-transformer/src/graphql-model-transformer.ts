...
import { BaseDataSource, CfnDataSource, LambdaDataSource } from '@aws-cdk/aws-appsync';
...
import {
  ...
  makeModelConditionInput,
  makeUpdateInputField,
  propagateApiKeyToNestedTypes,
  makeMutationModelInputType,
} from './graphql-types';
import {
...
export class ModelTransformer extends TransformerModelBase implements TransformerModelProvider {
  ...
  generateListResolver = (
    ...
    if (!this.resolverMap[resolverKey]) {
      this.resolverMap[resolverKey] = ctx.resolvers.generateQueryResolver(
        ...
        resolverLogicalId,
        dataSource,
        MappingTemplate.s3MappingTemplateFromString(generateListRequestTemplate(type.name.value), `${typeName}.${fieldName}.req.vtl`),
        MappingTemplate.s3MappingTemplateFromString(
  ...
  generateUpdateResolver = (
    ...
    if (!this.resolverMap[resolverKey]) {
      const resolver = ctx.resolvers.generateMutationResolver(
        ...
        dataSource,
        MappingTemplate.s3MappingTemplateFromString(
          generateUpdateRequestTemplate(type.name.value),
  ...
  generateDeleteResolver = (
    ...
    if (!this.resolverMap[resolverKey]) {
      this.resolverMap[resolverKey] = ctx.resolvers.generateMutationResolver(
        ...
        resolverLogicalId,
        dataSource,
        MappingTemplate.s3MappingTemplateFromString(generateDeleteRequestTemplate(type.name.value), `${typeName}.${fieldName}.req.vtl`),
        MappingTemplate.s3MappingTemplateFromString(
          generateDefaultResponseMappingTemplate(isSyncEnabled, true),
          `${typeName}.${fieldName}.res.vtl`,
        ),
  ...
  getInputs = (
    ...
    let inputType: InputObjectTypeDefinitionNode;
    if ([
      MutationFieldType.CREATE,
      MutationFieldType.DELETE,
      MutationFieldType.UPDATE,
      QueryFieldType.GET,
      QueryFieldType.LIST,
    ].includes(operation.type as MutationFieldType)) {
      const conditionTypeName = toPascalCase(['Model', type.name.value, 'ConditionInput']);
      const inputTypeName = toPascalCase(['Model', type.name.value, 'Input']);
      const filterInputs = createEnumModelFilters(ctx, type);
      conditionInput = makeModelConditionInput(ctx, conditionTypeName, type);
      inputType = makeMutationModelInputType(ctx, inputTypeName, type);

      filterInputs.push(conditionInput);
      filterInputs.push(inputType);
  ...
  getOutputType = (
    ...
    if (outputType?.name && !ctx.output.getObject(outputType.name.value)) {
  ...
  private createModelTable(stack: cdk.Stack, def: ObjectTypeDefinitionNode, context: TransformerContextProvider): void {
    ...
    this.createModelTableDataSource(def, context, stack);
  }
  ...
  private createModelTableDataSource(
    ...
    this.datasourceMap[def!.name.value] = dataSource as LambdaDataSource;
  }
